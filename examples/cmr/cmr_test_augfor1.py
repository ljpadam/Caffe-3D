...
for i in test_lst:
    ...
    shape_out = out1.shape
    for x in range(shape_out[1]):
      for y in range(shape_out[2]):
	for z in range(shape_out[3]):
	  if(out1[2,x,y,z]>out1[0,x,y,z] and out1[2,x,y,z]>out1[1,x,y,z]):
	    final[0][x,y,z]=256
	  else:
	    out1[1,x,y,z]=out1[1,x,y,z]/(out1[1,x,y,z]+out1[0,x,y,z])
	    if out1[1,x,y,z]>0.4:
	      final[0][x,y,z]=128
    #final[0][class1]=128
    #final[0][class2]=256
	...
	#print '/result/'+i[-1]+'/'+str(j)+'.png'
        ...
        #print out1.shape
        print j
        cv2.imwrite('./result/'+num+'/'+str(j)+'.png',final[0,j-int(start_lst[k]),:,:])
