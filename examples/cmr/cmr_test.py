...
with open('/home/ljp/code/caffe-3d/examples/cmr/test.txt') as f:   
...
caffemodel='/home/ljp/hd1/caffemodel/caffe3d/multi/45/cmr_iter_' + sys.argv[1]+'.caffemodel'
print caffemodel
net = caffe.Net('/home/ljp/code/caffe-3d/examples/cmr/test.prototxt', caffemodel, caffe.TEST)
caffe.set_mode_gpu()
caffe.set_device(0)
depth=200
k=0
for i in test_lst:
    ...
    #out1[out1>=0.5]=255
    #out1[out1<0.5]=0
    class1 = np.logical_and(out1[1,:,:,:] >= out1[0,:,:,:], out1[1,:,:,:] >= out1[2,:,:,:] )
    class2 = np.logical_and(out1[2,:,:,:] >= out1[0,:,:,:], out1[2,:,:,:] >= out1[1,:,:,:] )
    final[0][class1]=128
    final[0][class2]=256
    #out1=out1*255; 
    volume=net.blobs['data'].data[0]
	...
	num=i[-1]
    if not os.path.isdir('/home/ljp/code/caffe-3d/examples/cmr/result/'+num):
	os.mkdir('/home/ljp/code/caffe-3d/examples/cmr/result/'+num)
	...
	#print '/result/'+i[-1]+'/'+str(j)+'.png'
        ...
        #print out1.shape
        print j
        cv2.imwrite('/home/ljp/code/caffe-3d/examples/cmr/result/'+num+'/'+str(j)+'.png',final[0,j-int(start_lst[k]),:,:])
