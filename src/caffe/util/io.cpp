 ...
 *
 */
bool ReadVideoToCVMatSeg(const string& path,
    const int start_frame, const int length, const int height, const int width,
    const bool is_color, std::vector<cv::Mat>* cv_imgs, const bool img) {   //img=true: imge img=false: label
  ...
  // In case of a video file
    ...
    for (int i = start_frame; i <= end_frame; ++i) {
      if(img) snprintf(image_filename, sizeof(image_filename), "%s/img_%d.png", path.c_str(), i);
      else snprintf(image_filename, sizeof(image_filename), "%s/label_%d.png", path.c_str(), i);
      cv_img_origin = cv::imread(image_filename, CV_LOAD_IMAGE_ANYDEPTH);

      /*if(img) std::cout<<"img: "<<cv_img_origin.depth()<<" "<<cv_img_origin.channels()<<std::endl;
      else std::cout<<"label: "<<cv_img_origin.depth()<<" "<<cv_img_origin.channels()<<std::endl;*/
      if (!cv_img_origin.data) {
        LOG(ERROR) << "Could not read frame=" << i <<
                      " from an image file=" << image_filename;
        cv_imgs->clear();
